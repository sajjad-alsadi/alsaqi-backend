...
<p>يجب على جميع الموظفين والمتعاملين الالتزام بمدونة السلوك الوظيفي ومعايير النزاهة المعتمدة.</p>`
  ...
  const migrations = [
    ...
    // Strictly-increasing insertion sequence for the audit hash-chain. Provides
    // a deterministic, monotonic tiebreaker so the canonical writer
    // (AuditChainService) always chains off the true latest entry even when
    // multiple appends share the same millisecond timestamp (Requirement 7.2,
    // 7.3). BIGSERIAL installs an owned sequence whose default backfills any
    // existing rows and supplies a unique increasing value to every new insert.
    { table: "audit_trail", column: "seq", type: "BIGSERIAL" },
 ...
 * `runMigrations()` above. This registry is for incremental changes that should
 * only run once and be tracked in the schema_migrations table.
 *
 ...
 *
 * Consolidated here from the former `versionedMigrations.ts` module so that a
 * single migrations definition module owns both the base schema bootstrap
 * (`runMigrations`) and the versioned registry (`versionedMigrations`).
 */
